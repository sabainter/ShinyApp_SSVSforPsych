...
ui <- shinyUI(
  ...
  fluidPage(
    ...
    #titlePanel(h1("SSVSforPsych",h3("An interactive web tool for performing stochastic search variable selection"))),
...
server<-function(input, output, session) {
  ...
  output$resultsDF <- renderTable({
    ...
    withProgress(message = "Please wait, SSVS analysis is running", value=0.1, {
      ...
      names(resultsTable)<-c("Variable_name","Inclusion_probability","Standardized BMA Coefficient","Lower Credibility","Upper Credibility","Dependent_variable")
